...
type Directed[T types.Comparable] struct {
	...
	Source *vertex.Vertex[T]

	// Destination is the destination node in the edge
	Destination *vertex.Vertex[T]
...
func NewDirectedEdge[T types.Comparable](source, destination *vertex.Vertex[T], weight int) Edge[T] {
...
func (e *Directed[T]) GetNodes() []*vertex.Vertex[T] {
	return []*vertex.Vertex[T]{e.Source, e.Destination}
}
